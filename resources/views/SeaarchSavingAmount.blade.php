...
@section('content')
        ...
        <div class="row ">
            <div class="col-md-12">
                <div class="row d-flex justify-content-start">
                    <div class="col-4">
                        <button data-toggle="modal" data-target="#SearchModal" class="btn mt-2 btn-dark">Search</button>
                    </div>
            ...
            <div class="col-md-12  col-lg-12 col-sm-12 mt-4">
                ...
                <div class="card">
                    <div class="card-body">
                        <table id="SearchAmountDataTable" class="table table-bordered">
                            <thead>
                            <tr>
                                <th>Catagories</th>
                                <th>day</th>
                            ...
                            <tbody id="SearchAmountDataTableBody">
                            </tbody>
                        </table>
                    </div>
                </div>
    ...
    <div class="modal animat animate__fadeIn" id="SearchModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button id="SearchBtn" type="button" class="btn btn-dark">Confirm</button>
                </div>
            </div>
        </div>
    </div>
...
if(CatagoriesID.length==0){
    ...
    let URL="/searchSaveAmount/"+CatagoriesID+"/"+from_month+"/"+to_month+"/"+year;
    console.log(URL)




    $('#SearchBtn').html(BtnSpinner).prop("disabled", true);
    axios.get(URL).then(function (response) {
        $('#SearchBtn').html("Confirm").prop("disabled", false);
        if(response.status==200){
$('#total').html(response.data['tk']);
                    $('#LoadingDiv').addClass('d-none');
                    $('#MainDiv').removeClass('d-none')
                    $('#SearchAmountDataTable').DataTable().destroy();
                    $('#SearchAmountDataTableBody').empty();
                    ...
                    $.each(response.data['result'],function (i,item){
                        let tableRow="<tr>" +
                            ...
                            "</td>" +
                            "</tr>";
                        $('#SearchAmountDataTableBody').append(tableRow);
                    });
                    $('#SearchModal').modal('hide');
                    $('.deleteItem').on('click',function () {
                        let id=$(this).data('id');
                        $('#DeleteID').html(id);
                        $('#SearchModal').modal('hid');
                    })

                    $('#SearchAmountDataTable').DataTable({
                        "order":false,
